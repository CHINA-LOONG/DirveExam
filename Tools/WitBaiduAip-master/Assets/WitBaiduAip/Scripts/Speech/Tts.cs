...
namespace Wit.BaiduAip.Speech
{
    ...
    public class Tts : Base
    {
...
#if UNITY_STANDALONE || UNITY_EDITOR || UNITY_UWP||UNITY_WEBGL
        ...
        public IEnumerator Synthesis2(string text, Action<TtsResponse> callback, int speed = 5, int pit = 5, int vol = 5,
            Pronouncer per = Pronouncer.Female)
        {
            yield return PreAction();
            ...
            using (var uwr = UnityWebRequestMultimedia.GetAudioClip(url, AudioType.MPEG))
            {
                ...
                {
                    var type = uwr.GetResponseHeader("Content-Type");
                    if (type == "audio/mp3")
                    {
                    ...
                    else
                    {
                        ...
                        callback(JsonUtility.FromJson<TtsResponse>(uwr.downloadHandler.text));
                    }
                }
            }
        }
        ...
        IEnumerator _GetAudioClip(string url, Action<AudioClip> actionResult, AudioType audioType = AudioType.WAV)
        {
            using (var uwr = UnityWebRequestMultimedia.GetAudioClip(url, audioType))
            {
                ...
                {
                    var type = uwr.GetResponseHeader("Content-Type");
                    if (actionResult != null&& type == "audio/mp3")
                    {
